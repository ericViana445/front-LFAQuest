...
import AutomatonLesson, {
  extractAutomatonDetails,
  getNodesWithStates,
  getNosEspeciais,
...
import { FaXmark, FaBookOpen } from "react-icons/fa6"
...
const LessonTemplate: React.FC<LessonTemplateProps> = ({
  ...
  const [showTutorial, setShowTutorial] = useState(isAutomaton)
  ...
  const validateAutomatonEnhanced = (userEstados: Estado[], userConexoes: Conexao[]) => {
    ...
    const estadosIniciaisUsuario = [...nosPorTipoUsuario.iniciais, ...nosPorTipoUsuario.inicialEFinal].map(
      (e) => e.nome,
    )

    const estadosFinaisUsuario = [...nosPorTipoUsuario.finais, ...nosPorTipoUsuario.inicialEFinal].map((e) => e.nome)
    ...
    const estadoInicialCorreto = correctInicialStr === "" || estadosIniciaisUsuario.includes(correctInicialStr)

    const estadosFinaisCorretos =
      correctFinaisStr.length === 0 || correctFinaisStr.every((fim) => estadosFinaisUsuario.includes(fim))
    ...
    const conexoesUsuarioNormalizadas = userConexoes.map(normalizarConexao).sort((a, b) => {
    ...
    const conexoesCorretasNormalizadas = correctConexoes.map(normalizarConexao).sort((a, b) => {
      if (a.de !== b.de) return a.de.localeCompare(b.de)
      if (a.para !== b.para) return a.para.localeCompare(b.para)
      return a.caractere.localeCompare(b.caractere)
    })

    const conexoesIguais = JSON.stringify(conexoesUsuarioNormalizadas) === JSON.stringify(conexoesCorretasNormalizadas)
    ...
    if (!resultadoFinal) {
      ...
      if (!conexoesIguais) {
        ...
        const conexoesFaltando = conexoesCorretasNormalizadas.filter(
          (correct) =>
            !conexoesUsuarioNormalizadas.some(
        ...
        const conexoesExtras = conexoesUsuarioNormalizadas.filter(
          (user) =>
            !conexoesCorretasNormalizadas.some(
  ...
  const handleAutomatonValidation = (isValid: boolean, message: string, details: ValidationDetails) => {
    ...
    const estadosUnicos = todosEstados.filter(
      (estado, index, self) => index === self.findIndex((e) => e.id === estado.id),
    )
      ...
      try {
        ...
        if (user?.id) {
          ...
          if (!currentPhases.includes(String(nextPhase)) && nextPhase <= 5) {
            ...
            const res = await fetch(`https://backend-lfaquest.onrender.com/api/users/${user.id}/unlockedPhases`, {
              method: "PUT",
    ...
    return (
      <div className="summary-container">
        <div className="summary-card">
          <h1>
            <FaStar color="#facc15" /> Lição Concluída!
          </h1>
          ...
          {lessonResult && (
            <div className="reward-section">
              <p className="reward-text">
                <FaCoins className="inline-icon text-yellow-400" /> +{lessonResult.diamonds} moedas
              </p>
              <p className="reward-text">
                <FaCoins className="inline-icon text-yellow-400" /> +{lessonResult.diamonds} diamantes
              </p>
              <p className="reward-text fire-text">
                <WiDaySunny className="inline-icon text-yellow-900" /> Ofensiva atual: <b>{lessonResult.streak}</b> dia
          ...
          <div className="automaton-container">
            ...
            {isSubmitted && (
              <div className={`feedback ${isCorrect ? "correct-feedback" : "incorrect-feedback"}`}>
                {isCorrect ? (
                  <>
                    <FaStar color="#facc15" /> Parabéns! Autômato correto!
                  </>
                ) : (
                  <>Autômato incorreto! Tente novamente.</>
                )}

                {!isCorrect && userAutomaton && <div className="detailed-feedback"></div>}
              </div>
            )}
            ...
            <div className="action-buttons">
              {!isSubmitted ? (
                ...
                <button className="continue-button" onClick={handleContinue} disabled={isContinueClicked}>
                  {isContinueClicked ? "Processando..." : "Continuar →"}
                </button>
              )}
            </div>
          </div>
